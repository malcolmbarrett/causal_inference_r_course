...
**Ancestors of the exposure and outcome, minus any likely descendants**

```{r}
adjustmentSets(coffee_cancer_dag, type = "canonical")
```
...
saturated_dag_data |>
  ggplot(aes(x = x, y = y, xend = xend, yend = yend)) +
  geom_dag_edges(aes(edge_color = pruned), edge_width = 1, show.legend = FALSE) +
  geom_dag_point() +
  geom_dag_label_repel(aes(label = label)) +
  expand_plot(
    expand_x = expansion(c(0.2, 0.2)),
