...
### *Average Treatment Effect among the Unexposed / Controls (ATU / ATC)*
...
::: {.fragment}
For the ATE, the tilting function is $h(p_i) = 1$: the population is unchanged and the weights are unbounded
...
- Trimming and truncation respond to extreme propensity scores without saying where they came from
- **positively** searches the covariates for the region responsible

## Propensity scores with a violation
...
pos_df <- pos_model |>
  augment(type.predict = "response", data = pos_violations) |>
  mutate(w_ate = wt_ate(.fitted, exposure))

pos_df |>
  summarize(min = min(.fitted), max = max(.fitted), max_wt = max(w_ate))
```

## Which units did trimming drop?

```{r}
#| output-location: fragment

pos_df |>
  mutate(trimmed = is_unit_trimmed(ps_trim(.fitted, method = "adaptive"))) |>
  group_by(structural = region == "b" & x2 > 1) |>
  summarize(n = n(), trimmed = sum(trimmed))
```

. . .

Trimming dropped half of the 76 units that can never be exposed, and 396 units that can

## Locating the region directly

```{r}
#| output-location: fragment

check_port(pos_violations, exposure, c(x1, x2, region)) |>
  tidy() |>
  filter(low_support) |>
...
### *Stretch*: Using the same propensity scores, create ATM weights, plot the weighted distributions, and look for low-support regions with `check_port()`
