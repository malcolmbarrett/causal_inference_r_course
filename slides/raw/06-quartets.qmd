...
**Each of these datasets has nearly identical summary statistics: you must visualize your data to understand it**
...
# the numbers match the datasets, so they stay hidden until the reveal
dags +
  facet_wrap(
    ~dag,
    scales = "free",
    labeller = as_labeller(\(dag) str_remove(dag, "^\\(\\d\\) "))
  )
```
...
### Use `causal_quartet`, which stacks the 4 datasets; the exercise setup hides the names of their mechanisms

### For each of the 4 datasets, create a scatterplot looking at the relationship between `exposure` and `outcome`

### For each of the 4 datasets, look at the correlation between `exposure` and `covariate`

### *Stretch goal*: For each of the 4 datasets, fit a linear model to examine the relationship between the `exposure` and the `outcome`
...
causal_quartet |>
  ...
  mutate(dataset = paste("Dataset", as.integer(factor(dataset)))) |>
  group_by(dataset) |>
...
::: tiny
D'Agostino McGowan L, Gerke T, Barrett M (2023). Causal inference is not just a statistics problem.
Journal of Statistics and Data Science Education. doi:10.1080/26939169.2023.2276446
:::
...
effects |>
  mutate(percent_change = (ate_x - ate_xz) / ate_x) |>
  ...
  fmt_percent(columns = percent_change, decimals = 1) |>
  tab_options(table.font.size = "80%")
```

**Include a variable if it changes the effect estimate by more than ten percent**

. . .

**Every dataset trips the threshold, and adjusting is the wrong answer in some of them**
...
causal_quartet |>
  ...
  nest_by(dataset = as.integer(factor(dataset))) |>
  mutate(
    rmse = get_rmse(data, lm(outcome ~ exposure + covariate, data = data)) -
      get_rmse(data, lm(outcome ~ exposure, data = data)),
    r_squared = get_r_squared(lm(outcome ~ exposure + covariate, data = data)) -
      get_r_squared(lm(outcome ~ exposure, data = data))
  ) |>
  select(dataset, rmse, r_squared) |>
  ungroup() |>
  gt() |>
  gt::cols_label(
    dataset = "Dataset",
    rmse = "Change in RMSE",
    r_squared = md("Change in R^2^")
  ) |>
  fmt_number(columns = -dataset) |>
  tab_options(table.font.size = "80%")
```

**Adding the covariate improves the fit in every dataset**
...
### For each of the following 4 datasets, fit a linear model examining the relationship between `outcome_followup` and `exposure_baseline` adjusting for `covariate_baseline`: `causal_collider_time`, `causal_confounding_time`, `causal_mediator_time`, `causal_m_bias_time`
...
- Asymptotic theory shows that induction of M-bias is quite sensitive to various deviations from the exact M-Structure (Ding and Miratrix 2015)
:::

. . .

::: small
**If it is clearly M-bias, do not adjust for the variable. If it is not clear, adjust for it**: confounding bias tends to be worse, and M-structures are sensitive to slight deviations
:::
