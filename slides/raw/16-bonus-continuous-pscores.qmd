...
3. For a continuous exposure, we check correlations

## Correlations with `bal_corr()`

```{r}
balance_corrs <- nhefs_swts |>
  pivot_longer(
    c(age, smokeintensity, smokeyrs, wt71),
    names_to = "variable"
  ) |>
  summarize(
    observed = bal_corr(value, smkintensity82_71),
    weighted = bal_corr(value, smkintensity82_71, .weights = swts),
    .by = variable
  ) |>
  pivot_longer(
    c(observed, weighted),
...
### The weighted correlations are close to zero, so the weights are doing their job for these confounders
...
ggplot(
  balance_corrs,
  aes(x = correlation, y = variable, group = method, color = method)
) +
...
### Fit the same outcome model again, saving it to an object
...
dose_response <- nhefs_ipw |>
  ...
  predictions(
    newdata = datagrid(smkintensity82_71 = -25:25)
  )
