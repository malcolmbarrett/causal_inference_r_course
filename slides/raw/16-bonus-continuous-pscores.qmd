...
### In `wt_ate()`, calculate the weights using `.fitted`, `wait_minutes_posted_avg`, and the mean of `.sigma`
...
## Rebuild the weights for `ipw()`

```{r}
nhefs_ipw_wts <- nhefs_light_smokers |>
  mutate(swts = wt_ate(
    fitted(nhefs_model),
    smkintensity82_71,
...
### Rebuild the weights without `.sigma` and fit the outcome model with them

### Use `ipw()` to estimate the effect with standard errors that account for the weights
...
### `nhefs_ipw_wts` already has the weights that `ipw()` can rebuild from `nhefs_model`
