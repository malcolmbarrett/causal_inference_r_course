...
halfmoon::plot_mirror_distributions(
  net_data |>
    mutate(
      weighted = wts,
      net = factor(net, levels = c(FALSE, TRUE), labels = c("No", "Yes"))
    ),
  .fitted,
  .exposure = net,
  .weights = weighted
...
data_all_net <- net_data |>
  mutate(net = TRUE)

data_all_no_net <- net_data |>
  mutate(net = FALSE)

pred_net <- predict(outcome_model, newdata = data_all_net)
pred_no_net <- predict(outcome_model, newdata = data_all_no_net)

gcomp_data <- bind_rows(
  tibble(
    predicted_malaria_risk = pred_net,
    scenario = "All Treated"
  ),
  tibble(
    predicted_malaria_risk = pred_no_net,
...
ggplot(gcomp_data, aes(x = predicted_malaria_risk)) +
  geom_density(
    aes(fill = scenario),
    alpha = 0.7,
    color = "white",
...
```{r}
#| code-line-numbers: "|1,3,9|4-7|"

sl_library <- c("SL.glm", "SL.ranger", "SL.gam")

propensity_sl <- SuperLearner(
  Y = as.integer(net_data$net),
  X = net_data |>
    select(income, health, temperature) |>
...
propensity_scores <- predict(propensity_sl)$pred[, 1]

ate_weights <- wt_ate(propensity_scores, net_data$net)
...
```{r}
#| code-line-numbers: "|1,4,6,9|11-12|14"
...
pred_net <- predict(outcome_sl, newdata = data_all_net)$pred[, 1]
pred_no_net <- predict(outcome_sl, newdata = data_all_no_net)$pred[, 1]

mean(pred_net - pred_no_net)
```
...
## Targeted Learning: double robustness {.small}
...
## Targeted Learning: targeted estimation {.small}
...
## Targeted Learning: valid statistical inference {.small}
...
min_y <- min(net_data$malaria_risk)
max_y <- max(net_data$malaria_risk)
y_bounded <- (net_data$malaria_risk - min_y) / (max_y - min_y)
...
```{r}
initial_pred_net <- predict(outcome_sl_bounded, newdata = data_all_net)$pred[, 1]
initial_pred_no_net <- predict(outcome_sl_bounded, newdata = data_all_no_net)$pred[, 1]
...
# Use quasibinomial family and work on logit scale
...
logit_pred_net <- qlogis(initial_pred_net) + epsilon * (1 / propensity_scores)
logit_pred_no_net <- qlogis(initial_pred_no_net) + epsilon * (-1 / (1 - propensity_scores))

# Transform back to probability scale
targeted_pred_net <- plogis(logit_pred_net)
targeted_pred_no_net <- plogis(logit_pred_no_net)
```
...
targeting_data <- tibble(
  initial_net = initial_pred_net,
  targeted_net = targeted_pred_net,
  initial_no_net = initial_pred_no_net,
  targeted_no_net = targeted_pred_no_net
)

p_net <- ggplot(targeting_data, aes(x = initial_net, y = targeted_net)) +
  geom_point(alpha = 0.6, color = "#009E73") +
  geom_abline(slope = 1, intercept = 0, color = "red", linetype = "dashed") +
  labs(
    x = "Initial Predictions (Net Use)",
    y = "Targeted Predictions (Net Use)"
  ) +
...
p_no_net <- ggplot(
  targeting_data,
  aes(x = initial_no_net, y = targeted_no_net)
) +
  geom_point(alpha = 0.6, color = "#E69F00") +
  geom_abline(slope = 1, intercept = 0, color = "red", linetype = "dashed") +
  labs(
    x = "Initial Predictions (No Net)",
    y = "Targeted Predictions (No Net)"
  ) +
  theme_minimal(base_size = 14) +
  coord_fixed()
...
```{r}
initial_ate <- mean(
  initial_pred_net - initial_pred_no_net
  # Transform back to original scale for ATE
) *
  (max_y - min_y)

targeted_ate <- mean(
  targeted_pred_net - targeted_pred_no_net
) *
  (max_y - min_y)
...
targeted_pred_observed <- ifelse(
  net_data$net,
  targeted_pred_net,
  targeted_pred_no_net
)

# IC uses bounded outcomes and predictions
ic <- clever_covariate * (y_bounded - targeted_pred_observed) +
      targeted_pred_net - targeted_pred_no_net - targeted_ate / (max_y - min_y)

# Standard error on original scale
se_tmle <- sqrt(var(ic) / nrow(net_data)) * (max_y - min_y)
