...
- He contrasts it with the fate of his former crime partner, Spike, who was
  imprisoned for three years after a jewelry robbery and later sentenced to 35
  years to life for a botched robbery
...
- There are two potential outcomes, `y(continue)` and `y(quit)`, and only one
  will be realized
- This leaves one potential outcome observable and one missing; early causal
  inference methods were framed as missing data problems
...
## Does chocolate make you happier than vanilla?
...
Since we know each potential outcome, we can calculate the individual causal
effects
...
## Now we only know one potential outcome
...
We don't know the missing potential outcome, so we can't calculate the
individual causal effect

## Apples-to-apples

- Most of the assumptions we make for causal inference are to make an
  apples-to-apples comparison: we want to compare similar individuals who serve
  as reasonable proxies for each other's counterfactuals
...
Preference influences both the choice of flavor and the potential outcome

## Conditional exchangeability

We can still achieve exchangeability within levels of `prefer_chocolate`
...
But our sample size is so small that we have few values for each combination

## Positivity

- Every individual has a non-zero probability of receiving each level of
  exposure
...
Positivity must hold within levels of all covariates required for
exchangeability

## Positivity can fail within levels of a covariate
...
## Some potential outcomes are not defined
...
## The data now match their potential outcomes
...
## Be more specific about the potential outcomes
...
## Malaria nets and the causal assumptions
...
- In the limit, we expect exchangeability: the only cause of the exposure is the
...
- Observational studies have none of these guarantees, even ideal ones
...
The design of a causal analysis requires a clear causal question, which we map
to a protocol

- **Eligibility criteria**: who or what should be included in the study?
- **Exposure definition**: what precise exposure will units receive?
- **Assignment procedures**: how will units be assigned to an exposure?
...
## How the protocol helps the assumptions
...
- A precise exposure definition improves consistency and helps control
  interference
- Randomized assignment procedures meet exchangeability and positivity, and can
  prevent interference

## Emulating target trials

- There are many reasons we may not be able to randomize, including time, money,
  and ethical considerations
- You specify a target trial, the randomized trial you would run, and try to
  emulate this design with observational data
- The target trial we can emulate is usually a pragmatic trial, a trial without
  blinding
...
## Extra Magic Mornings at the Magic Kingdom
...
## Which days are missing an exposure group?
