...
data_observed_mixed_up <- data_observed |>
  mutate(
...
For stochastic violations, we can collect more data or use a statistical model
to extrapolate; for structural ones, we can add eligibility criteria or change
the estimand

## Finding positivity violations

- We planted the violations in these examples, so we knew where to look
- In real data, we have to search the covariates for regions where an exposure
  level is rare or absent

## A data set with two violations {.small}
...
- The tree locates the region; only background knowledge decides which kind of
  violation it is
- The search sees the covariates you give it, so it cannot find a violation
  defined by a variable you left out
...
```{r}
#| code-line-numbers: "|5-9|15-16|19-21"
#| output-location: fragment

set.seed(11)
data_observed_poorly_defined <- data |>
  mutate(
    y_spoiled_chocolate = 0,
...
set.seed(37)
data_observed_interf <- data_partners |>
  mutate(
...
partners_observed <- data_partners |>
...
### Fill in and run the positivity check with `check_port()`
