...
```{r}
initial_pred_net <- predict(outcome_sl_bounded, newdata = data_all_net)$pred[,
  1
]
initial_pred_no_net <- predict(
  outcome_sl_bounded,
  newdata = data_all_no_net
)$pred[, 1]
...
logit_pred_no_net <- qlogis(initial_pred_no_net) +
  epsilon * (-1 / (1 - propensity_scores))
