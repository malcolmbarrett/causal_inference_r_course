...
options(
  ...
  tibble.width = 60,
  # a cli rule at the default 80 characters clips on the rendered slides
  cli.width = 76
)
...
1. Fit a propensity model for `x ~ z`, where z is all covariates
2. Calculate weights
3. Fit an outcome model estimating the effect of `x` on `y` weighted by the propensity score

## Continuous exposures {background-color="#23373B"}
...
- NHEFS, the National Health and Nutrition Examination Survey Epidemiologic Follow-up Study, is a longitudinal, observational study
- Participants were surveyed and examined starting in 1971 and followed up through 1982
- Here we ask how the change in smoking intensity between the two visits affected weight gain
...
```{r}
nhefs_wts |>
  select(seqn, smkintensity82_71, .fitted, wts)
```
...
### We focus on the continuous confounders here; `check_balance()` expands categorical ones into indicator terms itself
...
### The estimate is the weight gain in kg for a 10-cigarette decrease
...
```{r}
# `group_level` repeats the exposure name or is empty, so drop it
balance_comparison |>
  select(-group_level)
```
...
4. ETA bias: simulate the estimator under the fitted models

## ETA bias

- Treat the fitted exposure and outcome models as the truth and simulate the estimator
- The bias that survives comes from positivity and sparsity alone, not model misspecification
- A red flag for the fitted mechanism, not a bias correction

## `check_eta_bias()`

- The diagnostic refits both models on each bootstrap draw, so it takes the confounders and the exposure model's formula
...
### The bias positivity contributes is one hundredth of the effect here

## Sweeping the weight cap
