...
In this example, we'll consider loss to follow-up in the NHEFS study. We'll use the question whose assumptions we examined in the causal assumptions exercise: does quitting smoking (`qsmk`) increase weight (`wt82_71`)? This time, however, we'll adjust for loss to follow-up (people who dropped out of the study between observation periods) using inverse probability of censoring weights.
...
2. Calculate the weights with `wt_cens()`. It takes the propensity score first, `.fitted`, and the censoring indicator second, `censored`
...
cens <- _______ |>
  ...
  mutate(cens_wts = wt_cens(______, ______)) |>
  select(id, cens_wts)
...
1. Create the cloned data sets, called `kept_smoking` and `quit_smoking`, where one dataset has `qsmk` set to 1 (quit smoking) and the other has it set to 0 (kept smoking).
2. Use the outcome model, `outcome_model`, to make predictions for `kept_smoking` and `quit_smoking`
...
fit_gcomp_cens <- function(.split, ...) {
  ...
  # add the censoring weights to `.df`: augment `.df` with the censoring model you
  # just fit, then mutate `cens_wts` with `wt_cens()`. no join here: every resample
  # needs its own weights, estimated on its own rows

  # fit outcome model. remember to model using the weighted data you just made from
  # `.df`, weighted by `cens_wts`, instead of `nhefs_censored_wts` or `nhefs_censored`

  # clone datasets. remember to clone the weighted data instead of `nhefs_censored`
