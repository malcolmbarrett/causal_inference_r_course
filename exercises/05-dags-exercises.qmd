...
2. We're going to assume that coffee does *not* cause cancer, so there's no formula for that. But we still need to declare our causal question. Specify "coffee" as the exposure and "cancer" as the outcome (both in quotation marks).
...
Time-ordering your DAGs is incredibly useful: the result is easier to read, and it is easier to understand what to control for, and what *not* to control for.

ggdag includes several ways to manually specify coordinates, including `time_ordered_coords()`. This function automatically determines the time ordering of the DAG based on the causal relationships. After all, something that causes something else *must* come before, otherwise we'd be violating the time-space continuum. 
