---
title: "Bonus: Machine Learning for Causal Inference"
format: html
---
...
2. Calculate the weights: for the ATE, `1 / propensity score` for treated units and `1 / (1 - propensity score)` for control units; `propensity::wt_ate()` will calculate this for you.
...
seven_dwarfs_all_treated <- seven_dwarfs |>
  mutate(park_extra_magic_morning = 1)

seven_dwarfs_all_control <- seven_dwarfs |>
  mutate(park_extra_magic_morning = 0)

# Step 3: Predict outcomes under each scenario
pred_treated <- predict(outcome_model, newdata = seven_dwarfs_all_treated)
pred_control <- predict(outcome_model, newdata = seven_dwarfs_all_control)
...
sl_library <- c(________, ________, ________)
...
```{r}
propensity_scores <- predict(______)$pred[, 1]
...
```{r}
# Step 4: Create the "clever covariate": this is the key to TMLE
...
6. Fit a fluctuation model with the bounded outcome, using `qlogis(initial_pred_observed)` as an offset and the clever covariate as a predictor (with no intercept). Use the quasibinomial family for the model.
7. Get the fluctuation parameter `epsilon` from the model coefficients; this is the coefficient for the clever covariate.

```{r}
# Step 5: The targeting step - a small parametric fluctuation of initial estimates
...
# Use quasibinomial family for bounded continuous outcomes;
...
8. Now that we've calculated the fluctuation parameter, we can update our predictions to obtain targeted predictions that minimize the bias-variance tradeoff for the average treatment effect.
   - For treated units, we add `epsilon * (1 / propensity_scores)`.
   - For control units, we add `epsilon * (-1 / (1 - propensity_scores))`.

```{r}
# Step 6: Update our predictions using the fluctuation parameter
...
9. Let's visualize the initial vs targeted individual-level predictions for treated and control units. First, collect the four sets of predictions into a data frame, `targeting_data`. Then set the x-axis to the initial predictions and the y-axis to the targeted predictions. For the first plot, use `initial_treated` and `targeted_treated`, and for the second plot, use `initial_control` and `targeted_control`.

```{r}
targeting_data <- tibble(
  initial_treated = initial_pred_treated,
  targeted_treated = targeted_pred_treated,
  initial_control = initial_pred_control,
  targeted_control = targeted_pred_control
)

# plot the initial vs targeted individual-level predictions for treated units
ggplot(targeting_data, aes(x = __________, y = __________)) +
  geom_point() +
...
1. Let's compare the mean difference in **initial** predictions between treated and control units, and the mean difference in **targeted** predictions between treated and control units. The first calculation is a g-computation estimate using the bounded outcome model, so it will differ slightly from your estimate in Your Turn 2. The second calculation is the TMLE estimate. Remember to transform back to the original scale!
...
# the mean difference in initial predictions is a g-computation estimate from the bounded outcome model
...
ic <- clever_covariate *
  (y_bounded - targeted_pred_observed) +
  targeted_pred_treated -
  targeted_pred_control -
  targeted_ate / (max_y - min_y)
