...
sim |>
  ...
  summarize(avg_y = mean(______), .groups = "drop") |>
  ...
  summarize(estimate = mean(estimate)) # note, we would need to weight this if the confounder groups were not equal sized
```
...
sim2 |>
  ...
  summarize(avg_y = mean(_____), .groups = "drop") |>
  pivot_wider(
    names_from = exposure,
    values_from = avg_y,
    names_prefix = "x_"
...
1. Run Your Turn 3 again with many more bins, say 500
2. The estimate is `NA`. Why?

```{r}
sim3 |>
  mutate(confounder_q = ntile(confounder, _____)) |>
  group_by(confounder_q, exposure) |>
  summarize(avg_y = mean(outcome), .groups = "drop") |>
  pivot_wider(
    names_from = exposure,
    values_from = avg_y,
    names_prefix = "x_"
...
- In simple cases, grouping by confounding variables can get us the right answer without a statistical model. This approach is called stratification, and it is non-parametric: we aren't using a statistical model to restrict the form of the variables
- With many confounders, especially continuous ones, we quickly run into the curse of dimensionality: too few observations within combinations of confounder levels
