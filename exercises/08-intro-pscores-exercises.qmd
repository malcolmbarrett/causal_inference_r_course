...
Model statistics can't tell us how well we've adjusted for confounding, but we do have expectations about how the model behaves.
First, check that the model is well-calibrated: among the days with a predicted probability of 0.3, about 30% should have had Extra Magic Hours in the morning.
Expect a warning about small sample sizes in some of the windows.
Only about 60 of these 354 days had Extra Magic Hours in the morning, so the windows at the upper end of the propensity score distribution (predicted probabilities above about 0.4) contain very few days; the warning tells you that the confidence band is imprecise there, not that you have done something wrong.
...
Next, look at how well the model discriminates between days with and without Extra Magic Hours in the morning.

```{r}
df |>
  check_model_roc_curve(____, ____) |>
  plot_model_roc_curve()
```

Finally, calculate the area under that curve.
Values between 0.6 and 0.8 usually mean we've picked up enough signal from the confounders to adjust for, but not so much that we have positivity problems.

```{r}
check_model_auc(df, .exposure = ____, .fitted = ____)
```

## Stretch goal

Examine two histograms of the propensity scores, one for days with Extra Magic Morning (`park_extra_magic_morning == 1`) and one for days without it (`park_extra_magic_morning == 0`).
